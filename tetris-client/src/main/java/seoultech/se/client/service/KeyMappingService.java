 ...
 * 사용 예시:
 * ```java
 * // GameSceneController에서
 * private void handleKeyPress(KeyCode key) {
 *     Optional<GameAction> action = keyMappingService.getAction(key);
 *     action.ifPresent(this::executeGameAction);
 * }
 * ```
 */
@Service
public class KeyMappingService {
     ...
     * @param action 게임 액션
     * @param keyCode 매핑할 키
     * @return 성공 여부 (이미 다른 액션에 사용 중이면 false)
     */
    ...
    private void setDefaultMappings() {
        ...
        setKeyMapping(GameAction.PAUSE, KeyCode.ESCAPE);
    }
